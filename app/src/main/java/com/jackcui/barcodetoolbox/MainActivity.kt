...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bd.fabClear.setOnClickListener {
            ...
            bd.tvOutput.text = SpannableStringBuilder().appendMySpan(WAIT_FOR_SCAN, "#7400FF", null)
            scanCnt = 1
            bd.extendedFabAction.visibility = View.GONE
        }
        ...
        when (scanTypeForm) {
            ...
            HmsScan.CONTACT_DETAIL_FORM -> {
                ...
                bd.extendedFabAction.setOnClickListener {
                    ...
                    // 姓名信息
                    peopleName?.run {
                        itt.putExtra(ContactsContract.Intents.Insert.NAME, getFullName())
                    }
                    // 公司信息
                    if (!company.isNullOrEmpty()) {
                        itt.putExtra(ContactsContract.Intents.Insert.COMPANY, company)
                    }
                    // 职位信息
                    if (!title.isNullOrEmpty()) {
                        itt.putExtra(ContactsContract.Intents.Insert.JOB_TITLE, title)
                    }
                    // 电话信息 + 邮箱信息 + 地址信息
                    itt.putParcelableArrayListExtra(ContactsContract.Intents.Insert.DATA, data)
                    // 备注信息
                    if (!note.isNullOrEmpty()) {
                        itt.putExtra(ContactsContract.Intents.Insert.NOTES, note)
                    }
                    startActivity(itt)
                }
                bd.extendedFabAction.visibility = View.VISIBLE
            }
